...
NC_nocov_gmm <- function(data, q = 10, alpha = 0.1) {
  ...
  bG <- - solve(t(S1) %*% S2 %*% t(S2) %*% S1 / spsz) %*% t(S1) %*% S2 
...
OLS_nocov_gmm <- function(data, q = 10, alpha = 0.1) {
  ...
  ATT.ols <- mean(Y[t0 + 1:t1] - Y0.ols[t0 + 1:t1])
  theta_est <- c(ATT.ols, ols.coef.pre)
...
scpi_ate_sc_int <- function(data, t0, method = "ols", alpha = 0.1) {
  Z <- data$Z
  W <- data$W
  Y <- data$Y
  tt <- length(Y)
...
scpi_ate_nc_int <- function(data, t0, method = "ols", alpha = 0.1) {
  Z <- data$Z
  W <- data$W
  Y <- data$Y
  tt <- length(Y)
