################################################################################
## SCPI prediction interval for average treatment effect over the post-treatment
## period
################################################################################

scpi.ate  <- function(data,
                      w.constr     = NULL,
                      V            = "separate",
                      V.mat        = NULL,
                      solver       = "ECOS",
  ...
  for (i in seq_len(I)) {
    ...
    loc.geom <- scpi:::local.geom(w.constr.list[[i]], rho, rho.max, res.list[[i]], B.list[[i]], 
    ...
    obj <- scpi:::u.des.prep(B.list[[i]], C.list[[i]], u.order, u.lags, coig.data[[i]],
    ...
    e.des <- scpi:::e.des.prep(B.list[[i]], C.list[[i]], P.list[[i]], e.order, e.lags,
  ...
  if (lgapp == "generalized") {
    ...
    reg.geom <- scpi:::local.geom.2step(w, r, rho.vec, rho.max, w.constr.list, Q, I)
    Q.star <- reg.geom$Q
    lb <- reg.geom$lb
  ...
  if (u.missp == TRUE) {
    ...
    for (i in seq_len(I)) {
      u.des.0.flex <- Matrix::bdiag(u.des.0.flex, 
                                    scpi:::DUflexGet(as.matrix(u.des.list[[i]]), C.list[[i]], f.id.list[[i]], M[[i]]))
  ...
  df <- scpi:::df.EST(w.constr = w.constr.list[[1]], w = w, B = B, J = Jtot, KM = KMI)
  if (df >= TT) {
    df <- TT - 1
  ...
  result <- scpi:::u.sigma.est(u.mean = u.mean, u.sigma = u.sigma, res = as.matrix(res.na),
                               Z = Z.na, V = V.na, index = index, TT = TT, df = df)
  
  Sigma <- result$Sigma
  Omega <- result$Omega
  Sigma.root <- scpi:::sqrtm(Sigma)
  ...
  if (w.lb.est == TRUE || w.ub.est == TRUE) {
    ...
    vsigg <- insampleUncertaintyGetATE(Z.na, V.na, P.na, beta, Sigma.root, J, KMI, I,
  ...
  if (w.lb.est == TRUE) {
    w.lb    <- apply(vsig[, 1, drop = FALSE], 2, quantile, probs = u.alpha / 2, na.rm = TRUE)
    fail.lb <- apply(vsigg[, 1, drop = FALSE], 2, function(x) sum(is.na(x)) / sims * 100)
  ...
  if (w.ub.est == TRUE) {
    w.ub    <- apply(vsig[, 2, drop = FALSE], 2, quantile,
                     probs = (1 - u.alpha / 2), na.rm = TRUE)
    fail.ub <- apply(vsigg[, 2, drop = FALSE], 2, function(x) sum(is.na(x))/sims*100)
  ...
  ## PIs for w
  sc.l.0 <- mean(Y.post.fit) + w.lb        # Left bound
  sc.r.0 <- mean(Y.post.fit) + w.ub        # Right bound
  ...
  if (w.constr.inf[[1]][["p"]] %in% c("L2","L1-L2")) {
    ...
    epsk <- scpi:::epskappaGet(P, rho.vec, beta.mat, I, sc.pred$data$specs$effect)
    epsk.j <- scpi:::epskappaGet(P, rho.vec, beta.mat, I, sc.pred$data$specs$effect, joint = TRUE)
  } else {
    epsk <- epsk.j <- 0
  }
  ...
  for (i in seq_len(I)) {
    e.des.0.na.list[[i]] <- scpi:::detectConstant(e.des.0.na.list[[i]])
    e.des.1.list[[i]] <- scpi:::detectConstant(e.des.1.list[[i]], scale.x)
  ...
  if (e.method == "gaussian" || e.method == "all") {
    ...
    pi.e   <- scpi.out.ate(res = e.res.na, x = e.des.0.na, eval = e.des.1,
...
insampleUncertaintyGetATE <- function(Z.na, V.na, P.na, beta, Sigma.root, J, KMI, I,
  ...
  # simulate
  ns <- scpi:::ECOS_get_n_slacks(w.constr.inf, Jtot, I)
  decomp <- scpi:::matRegularize(Q)
  Qreg <- decomp$Qreg
  scale <- decomp$scale
  dimred <- nrow(Q) - nrow(Qreg)
  
  data <- list()
  data[["dims"]] <- scpi:::ECOS_get_dims(Jtot, J, KMI, w.constr.inf, I, dimred)
  data[["A"]] <- scpi:::ECOS_get_A(J, Jtot, KMI, I, w.constr.inf, ns)
  data[["b"]] <- scpi:::ECOS_get_b(Q.star, Q2.star, w.constr.inf)
  
  if (cores == 1) {
    
    vsig <- matrix(NA, nrow = sims, ncol = 2)
    ...
    for (sim in seq_len(sims)) {
      ...
      data[["G"]] <- scpi:::ECOS_get_G(Jtot, KMI, J, I, a, Qreg, w.constr.inf, ns, dimred, scale)
      data[["h"]] <- scpi:::ECOS_get_h(d, lb, J, Jtot, KMI, I, w.constr.inf, Q.star, Q2.star, dimred)
      
      
      xt <- P.ave
      # minimization
      if (w.lb.est == TRUE) {
        data[["c"]] <- scpi:::ECOS_get_c(-xt, ns)
...
# Prediction interval, for e in the ATE
scpi.out.ate <- function(res, x, eval, e.method, alpha, e.lb.est, e.ub.est, verbose, effect, out.feat,
                         nperm = 1000) {
  ...
  if (e.lb.est == TRUE || e.ub.est == TRUE) {
    if (e.method == "gaussian") {
      x.more   <- rbind(eval, x)
      fit      <- scpi:::predict(y = res, x = x, eval = x.more, type = "lm")
      e.mean   <- fit[1:neval]
      res.fit  <- fit[-(1:neval)]
      
      if (effect == "time") {
        labels <- unlist(purrr::map(stringr::str_split(rownames(fit)[1:neval], "\\."), 2))
        e.mean <- scpi:::aggregateUnits(xx = e.mean, labels = labels)
      }
      
      var.pred <- scpi:::predict(y = log((res - res.fit)^2), x = x, eval = x.more, type = "lm")
      if (effect == "time") {
        var.pred <- scpi:::aggregateUnits(xx = var.pred[1:neval], labels = labels)
      } else {
        var.pred <- var.pred[1:neval]
      }
      e.sig2   <- exp(var.pred)
      
      q.pred <- scpi:::predict(y = res - res.fit, x = x, eval = x.more, type = "qreg", tau = c(0.25, 0.75))
      if (effect == "time") {
        q3.pred <- scpi:::aggregateUnits(xx = q.pred[1:neval, 2], labels = labels)
        q1.pred <- scpi:::aggregateUnits(xx = q.pred[1:neval, 1], labels = labels)
      } else {
      ...
      e.sig.mean <- sqrt(2 * sum(e.sig ^ 2) / length(e.sig) ^ 2)
      eps <- sqrt(-log(alpha)*2) * e.sig.mean
      ...
      ## get permutation distribution of the residuals
      res.perm <- replicate(nperm, mean(sample(res.st, neval)))
      ...
      e.sig.mean <- sqrt(sum(e.sig ^ 2) / length(e.sig) ^ 2)
      lb <- mean(e.mean) + e.sig.mean * quantile(res.perm, alpha)
      ub <- mean(e.mean) + e.sig.mean * quantile(res.perm, 1-alpha)
      ...
      perm_id <- replicate(nperm, sample(x = nrow(res), size = neval))
      res.perm <- matrix(NA, nrow = nperm, ncol = ncol(res))
      x.perm <- matrix(NA, nrow = nperm, ncol = ncol(x))
      
      for (pp in 1:nperm) {
        res.perm[pp, ] <- mean(res[perm_id[, pp], ])
        x.perm[pp, ] <- colMeans(x[perm_id[, pp], ])
      }
      
      eval_mean <- colMeans(eval)
      
      e.pred  <- scpi:::predict(y = res.perm, x = x.perm, 
                                eval = eval_mean, type="qreg", 
                                tau = c(alpha, 1 - alpha), verbose = verbose)
