...
library(parallel)
library(scpi)
source("myfunctions_Conformal.R")
...
run.one <- function(seed, grid, n.units, t0, mysd, theta, true.beta, addcov = F,
                    dist.epsilon, dist.lambda, U.setting, epsilonARMAcor = 0.1,
                    alpha = 0.1, grid_lb = -50, grid_ub = 50, output = "both"){
  set.seed(seed)
  .env <- environment()
  ...
  SC_CI_ub <- 
    ConformalPointwiseCI(data = data, t0 = t0, 
                         ...
                         method = "SC", alpha = alpha)
  ## SCPI 
  scpi_constrained <- scpi_int(data = data.full, t0 = t0, method = "simplex")
  scpi_unconstrained <- scpi_int(data = data.full, t0 = t0, method = "ols")
  ...
  return(c(NC_lb = NC_CI_lb, NC_ub = NC_CI_ub,
           ...
           scpi_constrained_lb = scpi_constrained[1], scpi_constrained_ub = scpi_constrained[2],
           scpi_unconstrained_lb = scpi_unconstrained[1], scpi_unconstrained_ub = scpi_unconstrained[2]))
}
